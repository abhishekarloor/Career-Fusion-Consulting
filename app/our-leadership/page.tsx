  ...
  return (
    <main className="min-h-screen bg-[#eef7fb] text-slate-900">
      ...
      <section className="relative overflow-hidden bg-[#003366] text-white">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,_rgba(0,164,204,0.25),transparent_55%)]" />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="max-w-2xl mb-12">
            <p className="text-sm uppercase tracking-[0.28em] text-[#7be1ff] mb-3">Our Leadership</p>
            <h1 className="text-5xl md:text-6xl font-bold tracking-tight text-white mb-6">
              Meet the leaders shaping Career Fusion.
            </h1>
            <p className="text-lg text-blue-100 leading-relaxed">
              Our leadership team drives strategy, culture, and client success with deep industry insight and a people-first approach.
            </p>
          </div>

          <div className="grid gap-8 xl:grid-cols-2">
            {leaders.map((leader) => (
              <article key={leader.name} className="mx-auto w-full max-w-xl overflow-hidden rounded-[32px] border border-white/10 bg-white shadow-2xl">
                <div className="mx-auto h-96 w-full overflow-hidden bg-slate-900 sm:h-[28rem]">
                  <img
                    src={assetPath(leader.image)}
                    alt={leader.name}
                    className="h-full w-full object-contain object-center bg-slate-900"
                  />
                </div>
                <div className="p-8 bg-white text-slate-900">
                  ...
                  <p className="text-slate-600 leading-8 mb-6">{leader.description}</p>
                  <p className="text-sm font-semibold text-slate-700">
                    Contact: <span className="text-[#003366]">{leader.contact}</span>
                  </p>
                </div>
              </article>
            ))}
